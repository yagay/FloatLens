...
Owns hide-icon / settle / capture / restore lifecycle. When LSPosed secure screenshot enhancement is
effective, it also owns the short-lived secure-capture authorization window: arm before backend capture,
wait briefly for target-process preference propagation, then clear on both success and failure. The remote
window also carries an absolute expiry so a missed cleanup cannot leave the bypass active indefinitely.
...
- `LsposedStatusManager` is the app-side owner of framework state and writable Remote Preferences. It
  mirrors `增强模式`, `LSPosed Provider`, LSPosed feature flags and short-lived capture requests into the
  `floatlens_runtime` group and separately tracks scope plus actually loaded target processes.
- `LsposedRuntimeConfig` is the shared key/schema and pure gate policy. The secure-capture gate requires
  enhanced mode + LSPosed Provider + feature switch + an unexpired capture deadline.
- `LsposedRuntimeProvider` is the hooked-process owner of read-only Remote Preferences. It keeps process-
  local gate state synchronized and exposes `canBypassSecureNow()`; installed hooks must call it at every
  behavior-changing invocation rather than relying on install-time state.
...
- The first functional LSPosed capability is controlled secure screenshot handling in `system_server`.
  `WindowState.isSecureLocked()` and compatible `ScreenCapture` native entry points are hooked, but remain
  behaviorally inert outside the short FloatLens capture window. Surface/window creation calls preserve
  the original secure result.
- SystemUI remains in recommended scope for status/future features; the secure screenshot capability does
  not modify SystemUI behavior.
- Never install a device-wide `system_server` behavior change that an in-app switch and automatic expiry
  cannot actually disable through this control path.

See `docs/PRIVILEGED_MODE.md` for the detailed provider and capture-window rules.
...
coordinate mapping, OCR early-stop quality, preference refresh routing, Circle state transitions, the
LSPosed two-switch runtime gate and secure-capture expiry/gating policy. Structural changes are not
complete until this workflow is green on the final HEAD.
...
14. Run the full CI gate after structural changes, then device-test: icon drag/dwell, View extraction,
    region screenshot, notification-shade capture, secure-content capture, screenshot->OCR update, native
