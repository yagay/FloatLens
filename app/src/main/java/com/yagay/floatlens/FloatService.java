...
public class FloatService extends Service implements android.content.SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    private WindowManager wm; private FvOverlayWindowHost iconHost; private FloatingIconLayoutPolicy layout; private FloatSettings fs; private CircleStateMachine circleState;
    ...
    private void addPrimary(){primaryLp=layout.createPrimary();primary=newIcon(primaryLp,false);primary.setAlpha(fs.alpha());addIconWindow(primary,primaryLp);layout.edgeHide(primaryLp);safeUpdate(primary,primaryLp);}
    private void syncSecondary(){if(fs.bothSide()){if(secondary==null&&primaryLp!=null){secondaryLp=layout.createMirror(primaryLp);secondary=newIcon(secondaryLp,true);secondary.setAlpha(fs.alpha());addIconWindow(secondary,secondaryLp);layout.edgeHide(secondaryLp);safeUpdate(secondary,secondaryLp);}}else if(secondary!=null){removeIconWindow(secondary);secondary=null;secondaryLp=null;}}
        ...
        return new FloatIconView(this,new FloatIconView.Callback(){
            ...
            @Override public void onRelease(boolean moved){
                ...
                if(positionMoveArmed&&moved){
                    if(fs.snap())snap(lp,icon);else{layout.edgeHide(lp);safeUpdate(icon,lp);}
                    if(mirrored&&primaryLp!=null){primaryLp.y=lp.y;layout.clamp(primaryLp,true);layout.edgeHide(primaryLp);safeUpdate(primary,primaryLp);}
                    ...
                    android.widget.Toast.makeText(FloatService.this,"移动图标位置已保存",android.widget.Toast.LENGTH_SHORT).show();
                    updateNotification();
                    return;
                }

                if(originReady[0]){lp.x=origin[0];lp.y=origin[1];layout.clamp(lp,true);safeUpdate(icon,lp);}
                else{layout.edgeHide(lp);safeUpdate(icon,lp);}
                if(!mirrored&&secondary!=null&&secondaryLp!=null){
                    if(mirrorOriginReady[0]){secondaryLp.x=mirrorOrigin[0];secondaryLp.y=mirrorOrigin[1];layout.clamp(secondaryLp,true);safeUpdate(secondary,secondaryLp);}
    ...
    private void syncMirrorPosition(){if(primaryLp==null||secondaryLp==null||secondary==null)return;layout.syncMirror(primaryLp,secondaryLp);safeUpdate(secondary,secondaryLp);}
    private void snap(WindowManager.LayoutParams lp,View v){layout.snapToVisibleEdge(lp);safeUpdate(v,lp);layout.edgeHide(lp);safeUpdate(v,lp);}
    ...
    public void onAccessibilityOverlayHostChanged(boolean available){getMainExecutor().execute(()->rehostIconWindows(available));}
    ...
    private void rehostOne(View v,WindowManager.LayoutParams lp,boolean useAccessibility){if(v!=null&&lp!=null&&iconHost!=null)iconHost.migrate(v,lp,useAccessibility,"float_icon_rehost");}
    ...
    private void restoreImeY(){if(imeRestoreY==null||primaryLp==null)return;primaryLp.y=imeRestoreY;layout.clamp(primaryLp,true);safeUpdate(primary,primaryLp);if(secondaryLp!=null){secondaryLp.y=primaryLp.y;layout.clamp(secondaryLp,true);safeUpdate(secondary,secondaryLp);}imeRestoreY=null;}
    ...
    public void refreshAppearance(){fs=new FloatSettings(this);layout.updateSettings(fs);int px=layout.iconPx();if(primary!=null&&primaryLp!=null){primaryLp.width=primaryLp.height=px;primary.setAlpha(fs.alpha());primary.refreshSettings();layout.clamp(primaryLp,true);layout.edgeHide(primaryLp);safeUpdate(primary,primaryLp);}if(secondary!=null&&secondaryLp!=null){secondaryLp.width=secondaryLp.height=px;secondary.setAlpha(fs.alpha());secondary.refreshSettings();layout.clamp(secondaryLp,true);layout.edgeHide(secondaryLp);safeUpdate(secondary,secondaryLp);}syncSecondary();appHidden=fs.shouldHideForPackage(topPackage);updateLockVisibility();updateImeAvoidance();recomputeVisibility();}
    @Override public void onSharedPreferenceChanged(android.content.SharedPreferences p,String key){refreshAppearance();}
    @Override public void onConfigurationChanged(Configuration c){layout.persist(primaryLp);super.onConfigurationChanged(c);imeRestoreY=null;removeIcons();fs=new FloatSettings(this);layout.updateSettings(fs);show();}
    ...
    @Override public void onDestroy(){layout.persist(primaryLp);if(circleState!=null)circleState.finish("service_destroy");removeIcons();try{fs.prefs().unregisterOnSharedPreferenceChangeListener(this);}catch(Throwable ignored){}if(screenReceiver!=null)try{unregisterReceiver(screenReceiver);}catch(Throwable ignored){}if(instance==this)instance=null;super.onDestroy();}
