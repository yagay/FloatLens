...
final class HookLogTransport {
    ...
    static void init(Context c){
        if(c!=null){
            ...
            synchronized(LOCK){if(!Q.isEmpty()&&!scheduled){scheduled=true;getHandler().post(HookLogTransport::flush);}}
        }
    }
    ...
    static void flush(){
        ...
        if(c==null){synchronized(LOCK){scheduled=true;}getHandler().postDelayed(HookLogTransport::flush,250);return;}
        StringBuilder b=new StringBuilder();int n=0;
        synchronized(LOCK){
            scheduled=false;
            while(!Q.isEmpty()&&n<40){String s=Q.pollFirst();if(b.length()+s.length()>24000){Q.addFirst(s);break;}b.append(s).append('\n');n++;}
            if(!Q.isEmpty()){scheduled=true;getHandler().postDelayed(HookLogTransport::flush,250);}
        }
        if(b.length()==0)return;
        try{Intent i=new Intent(ACTION_LOG).setPackage(TARGET);i.putExtra("payload",b.toString());c.sendBroadcast(i);}catch(Throwable t){
            synchronized(LOCK){if(Q.size()<300)Q.addFirst(System.currentTimeMillis()+" +"+SystemClock.uptimeMillis()+" [TRANSPORT_ERR] "+trim(String.valueOf(t),1200));}
        }
    }
