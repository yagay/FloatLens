...
public final class FvRuntimeModule extends XposedModule {
    ...
    private int hookMethod(Method m,String cat,boolean snapshot){String key=m.toGenericString();if(!hooked.add(key))return 0;try{m.setAccessible(true);hook(m).intercept(chain->{
            ...
            if(cat.equals("TOUCH") && chain.getArgs().stream().anyMatch(x->x instanceof MotionEvent e && e.getActionMasked()==MotionEvent.ACTION_MOVE)){long now=android.os.SystemClock.uptimeMillis();if(now-lastMoveLog<16)return chain.proceed();lastMoveLog=now;}
            ...
            HookLogTransport.log(cat,"ENTER "+HookFmt.member(m)+" args="+HookFmt.args(chain.getArgs())+" stack="+HookFmt.stack(5));
            Object result;try{result=chain.proceed();}catch(Throwable t){HookLogTransport.log(cat,"THROW "+HookFmt.member(m)+" "+t);throw t;}
            long us=(android.os.SystemClock.elapsedRealtimeNanos()-st)/1000;
            String diff=snapshot&&FvInspectorConfig.objectDiff?" diff="+ObjectSnapshot.diff(before,ObjectSnapshot.take(th)):"";
            HookLogTransport.log(cat,"EXIT "+HookFmt.member(m)+" result="+HookFmt.value(result)+" us="+us+diff);
            return result;
        });return 1;}catch(Throwable t){HookLogTransport.log("HOOK_ERR",key+" :: "+t);return 0;}}
