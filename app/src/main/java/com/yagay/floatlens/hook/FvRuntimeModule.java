...
public final class FvRuntimeModule extends XposedModule {
    ...
    @Override public void onPackageLoaded(XposedModuleInterface.PackageLoadedParam p){
        ...
        HookLogTransport.log("SELFTEST","target_loaded=true package="+p.getPackageName());
    ...
    private synchronized void install(ClassLoader cl){if(installed)return;installed=true;int n=0;
        ...
        n+=hookNamed(cl,"com.fooview.android.fooview.fvprocess.FooViewService",new String[]{"Y1","y3","R4","q4","h0","onCreate","onStartCommand","onDestroy","onConfigurationChanged"},"SERVICE",true);
        ...
        n+=hookAllExisting(cl,"com.fooview.android.fooview.fvprocess.CircleAppContainer","CIRCLE",false);
        n+=hookAllExisting(cl,"com.fooview.android.fooview.fvprocess.CircleGuideContainer","CIRCLE",false);
        ...
        HookLogTransport.log("INSTALL","hooks="+n);
        emitSelfTest(cl);
    ...
    private void emitSelfTest(ClassLoader cl){
        ...
        if(s!=null){for(String m:new String[]{"Y1","y3","R4","q4","h0"})HookLogTransport.log("SELFTEST","method=FooViewService."+m+" found="+hasMethod(s,m));}
    ...
    private int hookMethod(Method m,String cat,boolean snapshot){String key=m.toGenericString();if(!hooked.add(key))return 0;try{m.setAccessible(true);hook(m).intercept(chain->{boolean enabled=enabled(cat); if(!enabled)return chain.proceed();if(cat.equals("TOUCH") && chain.getArgs().stream().anyMatch(x->x instanceof MotionEvent e && e.getActionMasked()==MotionEvent.ACTION_MOVE)){long now=android.os.SystemClock.uptimeMillis();if(now-lastMoveLog<16)return chain.proceed();lastMoveLog=now;}Object th=chain.getThisObject();Map<String,String> before=snapshot&&FvInspectorConfig.objectDiff?ObjectSnapshot.take(th):Map.of();long st=android.os.SystemClock.elapsedRealtimeNanos();HookLogTransport.log(cat,"ENTER "+HookFmt.member(m)+" args="+HookFmt.args(chain.getArgs())+" stack="+HookFmt.stack(5));Object result;try{result=chain.proceed();}catch(Throwable t){HookLogTransport.log(cat,"THROW "+HookFmt.member(m)+" "+t);throw t;}long us=(android.os.SystemClock.elapsedRealtimeNanos()-st)/1000;String diff=snapshot&&FvInspectorConfig.objectDiff?" diff="+ObjectSnapshot.diff(before,ObjectSnapshot.take(th)):"";HookLogTransport.log(cat,"EXIT "+HookFmt.member(m)+" result="+HookFmt.value(result)+" us="+us+diff);return result;});HookLogTransport.log("SELFTEST","hooked="+HookFmt.member(m));return 1;}catch(Throwable t){HookLogTransport.log("HOOK_ERR",key+" :: "+t);return 0;}}
