...
public final class FvRuntimeModule extends XposedModule {
    ...
    private int hookL3Downstream(ClassLoader cl){Class<?> c=find(cl,"com.fooview.android.fooview.fvprocess.FooViewService");if(c==null)return 0;int n=0;for(Method m:c.getDeclaredMethods()){if(m.isSynthetic())continue;String mn=m.getName();if(mn.equals("i3")||mn.equals("k3")||mn.equals("l3")||mn.equals("Y1"))continue;String key="l3downstream:"+m.toGenericString();if(!hooked.add(key))continue;try{m.setAccessible(true);hook(m).intercept(chain->{if(calledFrom("l3"))FvSessionTracker.onDownstream(m.getDeclaringClass().getSimpleName(),m.getName(),chain.getArgs());return chain.proceed();});n++;}catch(Throwable t){HookLogTransport.log("HOOK_ERR","l3downstream "+m.toGenericString()+" :: "+t);}}return n;}
