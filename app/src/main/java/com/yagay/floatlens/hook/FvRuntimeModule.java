...
public final class FvRuntimeModule extends XposedModule {
    ...
    private int hookMethod(Method m,String cat,boolean snapshot){String key=m.toGenericString();if(!hooked.add(key))return 0;try{m.setAccessible(true);hook(m).intercept(chain->{
        ...
        if(dc.endsWith("FooViewService$c3") && mn.equals("b")) {Integer code=firstInt(args); if(code!=null){FvSessionTracker.onCode(code,"b");HookLogTransport.log("FV_CODE","source=b code="+code+" label="+FvSessionTracker.label(code));}}
        if(dc.endsWith("FooViewService") && mn.equals("Y1")) {Integer code=firstInt(args); if(code!=null){FvSessionTracker.onCode(code,"Y1");HookLogTransport.log("FV_CODE","source=Y1 code="+code+" label="+FvSessionTracker.label(code));}}
        ...
        HookLogTransport.log(cat,"ENTER "+HookFmt.member(m)+" args="+HookFmt.args(args)+" stack="+HookFmt.stack(5));
