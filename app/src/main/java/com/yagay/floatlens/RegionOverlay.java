...
public final class RegionOverlay {
    public static void show(Context c, Bitmap screen, boolean ocr) {
        if(ocr){FloatService f=FloatService.get();if(f!=null)f.onCircleCaptureStarted();}
    ...
    static class SelectView extends View {
        ...
        @Override public boolean onTouchEvent(MotionEvent e){switch(e.getActionMasked()){case MotionEvent.ACTION_DOWN->{sx=ex=e.getX();sy=ey=e.getY();selecting=true;pts.clear();lasso.reset();lasso.moveTo(sx,sy);pts.add(new PointF(sx,sy));invalidate();return true;}case MotionEvent.ACTION_MOVE->{ex=e.getX();ey=e.getY();if(ocr){lasso.lineTo(ex,ey);pts.add(new PointF(ex,ey));}invalidate();return true;}case MotionEvent.ACTION_UP->{ex=e.getX();ey=e.getY();if(ocr){lasso.lineTo(ex,ey);lasso.close();pts.add(new PointF(ex,ey));}finishSelection();return true;}case MotionEvent.ACTION_CANCEL->{if(ocr){FloatService f=FloatService.get();if(f!=null)f.onCircleFinished("selection_cancel");}close();return true;}}return true;}
        ...
        private void finishSelection(){RectF r=ocr?lassoBounds():rect();close();if(r.width()<dp(8)||r.height()<dp(8)){if(ocr){FloatService f=FloatService.get();if(f!=null)f.onCircleFinished("selection_too_small");}return;}float xs=b.getWidth()/(float)getWidth(),ys=b.getHeight()/(float)getHeight();int x=Math.max(0,Math.round(r.left*xs)),y=Math.max(0,Math.round(r.top*ys));int w=Math.min(b.getWidth()-x,Math.max(1,Math.round(r.width()*xs))),h=Math.min(b.getHeight()-y,Math.max(1,Math.round(r.height()*ys)));try{Bitmap crop=Bitmap.createBitmap(b,x,y,w,h);if(ocr){Bitmap masked=Bitmap.createBitmap(w,h,Bitmap.Config.ARGB_8888);Canvas mc=new Canvas(masked);mc.drawColor(Color.WHITE);Path path=new Path();boolean first=true;for(PointF q:pts){float px=q.x*xs-x,py=q.y*ys-y;if(first){path.moveTo(px,py);first=false;}else path.lineTo(px,py);}path.close();mc.save();mc.clipPath(path);mc.drawBitmap(crop,0,0,null);mc.restore();FloatService f=FloatService.get();if(f!=null)f.onCircleRecognizeStarted();OcrEngine.recognize(getContext(),masked);}else ScreenshotController.save(getContext(),crop);}catch(Throwable t){if(ocr){FloatService f=FloatService.get();if(f!=null)f.onCircleFinished("selection_error");}Toast.makeText(getContext(),"区域处理失败: "+t.getMessage(),Toast.LENGTH_LONG).show();}}
