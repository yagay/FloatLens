 ...
 * The original FloatIconView remains visible/alive as the event owner so Android does not cancel the
 * pointer stream when Circle starts. The full-screen Circle layer is deliberately NOT_TOUCHABLE.
 */
public final class CircleLiveController {
    ...
    public static synchronized void finish(float x, float y) { Session s=active;active=null;if(s!=null)s.finish(x,y,false); }
    public static synchronized void cancel(String reason) { Session s=active;active=null;if(s!=null)s.cancel(reason==null?"cancel":reason); }
    ...
    private static final class Session {
        ...
        void finish(float x,float y,boolean wasCancelled){
            if(ended)return;
            ended=true;cancelled=wasCancelled;points.add(new PointF(x,y));
        ...
        void cancel(String reason){
            ...
            ended=true;cancelled=true;closeOverlay();
            FloatService f=FloatService.get();if(f!=null)f.onCircleFinished(reason);
        ...
        private void onCaptureFailure(Throwable t){
            closeOverlay();
            FloatService f=FloatService.get();if(f!=null)f.onCircleFinished("capture_failed");
        ...
        private void captureScreen(Consumer<Bitmap> ok,Consumer<Throwable> fail){
            ...
            if(fs.accessibilityScreenshot()&&a!=null){
                a.capture(ok,accessError->{if(fs.rootScreenshot())RootCapture.captureAsync(context,ok,fail);else fail.accept(accessError);});
                return;
            }
            if(fs.rootScreenshot()){
                RootCapture.captureAsync(context,ok,rootError->{if(a!=null)a.capture(ok,fail);else fail.accept(rootError);});
                return;
            }
            if(a!=null)a.capture(ok,fail);else fail.accept(new IllegalStateException("需要开启 FloatLens 无障碍截图或 Root 截图"));
        }
        ...
        private void maybeProcess(){
            ...
            processed=true;closeOverlay();
            if(cancelled)return;
            Bitmap masked=createMaskedCrop();
            if(masked==null){FloatService f=FloatService.get();if(f!=null)f.onCircleFinished("selection_too_small");return;}
        ...
        private Bitmap createMaskedCrop(){
            ...
            float sx=screenshot.getWidth()/(float)Math.max(1,display.width()),sy=screenshot.getHeight()/(float)Math.max(1,display.height());
            ...
            int x=Math.max(0,Math.min(screenshot.getWidth()-1,Math.round(minX*sx))),y=Math.max(0,Math.min(screenshot.getHeight()-1,Math.round(minY*sy)));
            int right=Math.max(x+1,Math.min(screenshot.getWidth(),Math.round(maxX*sx))),bottom=Math.max(y+1,Math.min(screenshot.getHeight(),Math.round(maxY*sy)));
            int w=right-x,h=bottom-y;if(w<=1||h<=1)return null;
            Bitmap crop=Bitmap.createBitmap(screenshot,x,y,w,h),masked=Bitmap.createBitmap(w,h,Bitmap.Config.ARGB_8888);
            Canvas canvas=new Canvas(masked);canvas.drawColor(Color.WHITE);
            Path path=new Path();boolean first=true;
            for(PointF p:points){float px=p.x*sx-x,py=p.y*sy-y;if(first){path.moveTo(px,py);first=false;}else path.lineTo(px,py);}path.close();
            canvas.save();canvas.clipPath(path);canvas.drawBitmap(crop,0,0,null);canvas.restore();crop.recycle();
    ...
    private static final class LiveView extends View {
        ...
        private final Paint bitmapPaint=new Paint(Paint.ANTI_ALIAS_FLAG),shade=new Paint(Paint.ANTI_ALIAS_FLAG),line=new Paint(Paint.ANTI_ALIAS_FLAG),text=new Paint(Paint.ANTI_ALIAS_FLAG);
        private List<PointF> points;
        LiveView(Context c,Bitmap b,List<PointF> p){super(c);screenshot=b;points=new ArrayList<>(p);shade.setColor(0x66000000);shade.setStyle(Paint.Style.FILL);line.setColor(Color.WHITE);line.setStyle(Paint.Style.STROKE);line.setStrokeWidth(dp(3));text.setColor(Color.WHITE);text.setTextSize(dp(15));text.setShadowLayer(dp(3),0,dp(1),Color.BLACK);}
        void setPoints(List<PointF> p){points=new ArrayList<>(p);invalidate();}
        @Override protected void onDraw(Canvas c){super.onDraw(c);c.drawBitmap(screenshot,null,new Rect(0,0,getWidth(),getHeight()),bitmapPaint);c.drawRect(0,0,getWidth(),getHeight(),shade);if(points!=null&&!points.isEmpty()){Path path=new Path();boolean first=true;for(PointF p:points){if(first){path.moveTo(p.x,p.y);first=false;}else path.lineTo(p.x,p.y);}c.drawPath(path,line);}c.drawText("圈选内容 · 松手识别",dp(18),dp(38),text);}
